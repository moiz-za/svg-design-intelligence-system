...
**Conscious Tradeoff & Policy Origin Authority:**
This native embedding introduces a deliberate content duplication across two repositories (~13 files copied from `etsy-seller-seo-system` into `integration/etsy-seo-engine/`). 
- **Accepted Tradeoff:** Single-pass LLM execution reliability outweighs strict single-source DRY purity. LLMs execute inline instructions far more accurately than external cross-repo pointers.
- **Policy Origin Authority & Automated Dual-Repo Sync:** To prevent silent drift between `etsy-seller-seo-system` and `svg-design-intelligence-system`, an automated sync engine (`scripts/sync_etsy_policy.py`) runs during Phase 1 policy freshness checks. It performs bidirectional mtime and MD5 hash comparisons across both repositories (`integration/etsy-seo-engine/` ↔ `skill/references/`), copying newer edits instantly and rebuilding skill packages (`esvg-dis.skill` & `etsy-seller.skill`) with zero manual effort.
