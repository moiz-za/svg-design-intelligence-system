...
1. **Policy & Algorithm Freshness Check & Automated Sync:** Verify output against August 11, 2026 Etsy Creativity Standards. Run `python3 scripts/sync_etsy_policy.py` to auto-sync policy rulebooks bidirectionally between ESVG-DIS (`integration/etsy-seo-engine/`) and Etsy Seller SEO System (`skill/references/`) to guarantee zero policy drift.
